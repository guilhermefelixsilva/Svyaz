...
namespace Api.Service.Services.Odoo.Configuration.New.CRM
{
    public class NewconfigurationCRM
    {
        ...
        public NewconfigurationCRM(string CustomerEmail, string CustomerTag)
        {
        ...
        private void Newodooconf() //Odoo.conf
        {
            ...
            writer.WriteLine("logfile = " + Path.GetFullPath(basePath).Substring(0, 5) + @"/" + customerEmail + @"/Logs" + @"/" + "CRM" + @"/odoo-server" + ".log");
        ...
        {
            logPath = Path.GetFullPath(basePath).Substring(0, 5) + @"/" + customerEmail + @"/Logs" + @"/" + "CRM";
