...
namespace Api.Service.Services.Odoo.Configuration.New.Faturamento
{
    public class NewconfigurationFaturamento
    {
        ...
        private void Newodooconf() //Odoo.conf
        {
            ...
            configPath = Path.GetFullPath(basePath).Substring(0, 5) + @"/" + customerEmail + @"/FATURAMENTO/" + customerEmail + "_" + customerTag;
            ...
            writer.WriteLine("logfile = " + Path.GetFullPath(basePath).Substring(0, 5) + @"/" + customerEmail + @"/Logs" + @"/" + "Faturamento" + @"/odoo-server" + ".log");
        ...
        {
            logPath = Path.GetFullPath(basePath).Substring(0, 5) + @"/" + customerEmail + @"/Logs" + @"/" + "Faturamento";
